...
import { 
  Bell, Trophy, Calendar, CheckCircle, AlertCircle, 
  BarChart2, Droplets, Moon, Smartphone, Award, ArrowUp, 
  ArrowDown, Minus, Plus, Settings, X, Info
} from "lucide-react";
...
const habits = [
  { 
    ...
    tips: ["Avoid screens before bed", "Keep a consistent sleep schedule", "Make your bedroom cool and dark"]
  },
  { 
    name: "Water", 
    unit: "L", 
    ...
    tips: ["Carry a water bottle", "Drink a glass with each meal", "Set reminders throughout the day"]
  },
  { 
    name: "Screen Time", 
    unit: "hrs", 
...
export default function HabitTracker() {
  const [today, setToday] = useState<{ [key: string]: number }>({ Sleep: 0, Water: 0, "Screen Time": 0 });
  const [streaks, setStreaks] = useState<{ [key: string]: number }>({ Sleep: 0, Water: 0, "Screen Time": 0 });
  ...
  const getAchievementDetails = (key: string) => {
    ...
    return achievementDetails[key] || { title: "Achievement Unlocked", description: "You've reached a milestone!" };
  };
      ...
      <header className="mb-8">
        <div className="flex items-center justify-center mb-4">
          ...
          <h2 className="text-lg font-medium text-gray-600">
            {daysOfWeek[currentDate.getDay()]}, {currentDate.getDate()} {getMonthName(currentDate.getMonth())} {currentDate.getFullYear()}
          </h2>
        </div>
        <h1 className="text-4xl font-bold text-center mb-2 bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Habit Tracker</h1>
        <p className="text-gray-500 text-center max-w-lg mx-auto">Track your daily habits, analyze trends, and build consistent routines</p>
      </header>
          ...
          >
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-amber-600">You didn't meet your {reminderHabit} goal yesterday. Let's get back on track today!</p>
              </div>
            </div>
            <button 
    ...
    >
        ...
        <div className="inline-flex rounded-md shadow-sm bg-white p-1">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${
              activeTab === "daily" ? "bg-blue-500 text-white shadow-md" : "text-gray-700 hover:bg-gray-100"
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${
              activeTab === "analytics" ? "bg-blue-500 text-white shadow-md" : "text-gray-700 hover:bg-gray-100"
            }`}
          >
          ...
          {Object.entries(achievements).map(([key, unlocked]) => (
            <div 
              ...
              className={`flex-shrink-0 snap-start rounded-lg p-3 ${
                unlocked ? "bg-gradient-to-br from-yellow-400 to-amber-500 text-white" : "bg-gray-200 text-gray-400"
              } min-w-32 relative overflow-hidden`}
            >
              <div className="flex items-center">
                <Award className={`w-5 h-5 mr-2 ${unlocked ? "text-yellow-100" : "text-gray-400"}`} />
                <div>
                  <h4 className="font-medium text-sm">{getAchievementDetails(key).title}</h4>
                  {unlocked && <p className="text-xs text-yellow-100">Unlocked!</p>}
                  {!unlocked && <p className="text-xs">Locked</p>}
                </div>
              </div>
              {!unlocked && <div className="absolute inset-0 bg-gray-200 bg-opacity-50 backdrop-blur-sm" />}
            </div>
          ))}
        </div>
      </div>
      ...
      {activeTab === "daily" && (
        <div className="grid gap-6 md:grid-cols-3">
          {habits.map(({ name, unit, goal, icon, color, description, tips }) => (
            <motion.div 
              key={name}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  <div className={`p-2 rounded-lg ${getColorClass(color, "bgLight")} mr-3`}>
                    {icon}
                  </div>
                  <div>
              ...
              <AnimatePresence>
                {showTips && selectedHabit === name && (
                  ...
                  >
                    <h4 className="font-medium text-sm mb-2 text-gray-700">Tips to improve your {name.toLowerCase()}:</h4>
              ...
              <div className="relative pt-1">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <span className={`text-xs font-semibold inline-block ${getColorClass(color, "text")}`}>
                      Progress: {Math.round((today[name] / goal) * 100)}%
                    </span>
                  </div>
                  <div className={`text-xs font-semibold inline-block ${getColorClass(color, "text")}`}>
                ...
                <div className="overflow-hidden h-2 text-xs flex rounded bg-gray-200">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.min((today[name] / goal) * 100, 100)}%` }}
                    transition={{ duration: 0.5 }}
                    className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center ${getColorClass(color, "bg")}`}
                  ></motion.div>
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between items-center">
                  <div className="flex justify-between text-xs text-gray-500">
                    <span>0{unit}</span>
                    <span className="mx-auto">{goal}{unit}</span>
                    <span>{goal * 2}{unit}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <button 
                      onClick={() => handleChange(name, Math.max(0, today[name] - (name === "Water" ? 0.1 : 0.5)))}
                      className={`w-6 h-6 rounded-full flex items-center justify-center ${getColorClass(color, "bgLight")} ${getColorClass(color, "text")}`}
                    >
                      <Minus className="w-3 h-3" />
                    </button>
                    <button 
                      onClick={() => handleChange(name, Math.min(goal * 2, today[name] + (name === "Water" ? 0.1 : 0.5)))}
                      className={`w-6 h-6 rounded-full flex items-center justify-center ${getColorClass(color, "bgLight")} ${getColorClass(color, "text")}`}
                    >
          ...
          {habits.map(({ name, unit, goal, icon, color }) => (
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex space-x-4">
                  <div className="text-center">
                    ...
                    <p className={`font-bold ${getColorClass(color, "text")}`}>{getWeeklyAverage(name)}{unit}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-gray-500">Completion</p>
                    <p className="font-bold text-green-600">{getCompletionRate(name)}%</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-gray-500">Trend</p>
                    <p className={`font-bold flex items-center ${
                      getTrend(name) === "up" ? "text-green-600" : 
                      getTrend(name) === "down" ? "text-red-600" : 
                      "text-gray-600"
                    }`}>
                      {getTrend(name) === "up" ? <ArrowUp className="w-4 h-4" /> : 
                       getTrend(name) === "down" ? <ArrowDown className="w-4 h-4" /> : 
                        ...
                        <div className="flex justify-center">
                          <motion.div
                            ...
                            className={`w-full max-w-md rounded-t-lg ${
                              val >= goal ? getColorClass(color, "bg") : "bg-gray-300"
                            }`}
                            style={{ minHeight: "4px" }}
                          >
                ...
                <div className="flex space-x-2">
                  {daysOfWeek.map((day, i) => (
                    <div key={day} className="flex-1 text-center">
                      <span className={`text-xs ${i === new Date().getDay() ? `font-bold ${getColorClass(color, "text")}` : "text-gray-500"}`}>
                        {day}
                      </span>
                    </div>
                  ))}
              ...
              <div className="mt-6 pt-4 border-t border-gray-100">
                <h4 className="text-sm font-medium text-gray-500 mb-3">Performance Insights</h4>
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <p className="text-xs text-gray-500 mb-1">Best Day</p>
                    <p className="font-medium">
                      {daysOfWeek[mockWeeklyData[name].indexOf(Math.max(...mockWeeklyData[name]))]}
                    </p>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <p className="text-xs text-gray-500 mb-1">Consistency</p>
                    <p className="font-medium">
                      {Math.round(100 - (Math.max(...mockWeeklyData[name]) - Math.min(...mockWeeklyData[name])) / goal * 50)}%
                    </p>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <p className="text-xs text-gray-500 mb-1">Days Above Goal</p>
                    <p className="font-medium">
